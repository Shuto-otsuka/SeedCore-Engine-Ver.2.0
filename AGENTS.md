...
- **Do not edit `.vcxproj`/`.vcxproj.filters` files.** New files are registered into the project by the user, not by Codex — when a new source file is added, leave it out of the `.vcxproj`; don't add `<ClCompile>`/`<ClInclude>` entries yourself.
...
- **Doc comments — split by what's being commented, not by comment length:**
  ...
  Note: the AGENTS.md file itself uses plain `//` for its own code-block comments; `///` is the source doc-comment marker, not a formatting hint for this file.
