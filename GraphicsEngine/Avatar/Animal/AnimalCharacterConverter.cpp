...
namespace SeedCore
{
	void AnimalCharacterConverter::Convert(const AnimalCharacterModel& model, const AnimalCharacterEvaluator& evaluator, tinygltf::Model& outModel)
	{
		...
		std::span<const Char* const> regionNames = AnimalCharacterModel::RegionNames();
		...
		{
			const AnimalCharacterRegionRange& regionRange = model.RegionTriangleRange(regionIndex);
		...
		std::span<const Char* const> boneNames = AnimalCharacterModel::BoneNames();
		...
		for (Uint32 boneIndex = 0; boneIndex < model.BoneCount(); boneIndex++)
		{
			const AnimalCharacterBone& bone = model.Bone(boneIndex);
	...
	Bool AnimalCharacterConverter::Bake(const AnimalCharacterModel& model, const AnimalCharacterEvaluator& evaluator, ExportPreset preset, String filePath)
	{
