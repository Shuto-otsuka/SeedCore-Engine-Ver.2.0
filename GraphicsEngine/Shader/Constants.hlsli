...
// scheme as ShadowAccumulationIndices above, extended with a hit-point virtual
// motion reprojection candidate (see ReflectionDenoiseCS.hlsl). The raw 1spp
// GGX-sampled radiance (structured_indices.reflection_) is shared/single-
// buffered across views; this chain is the per-view denoised result
// DeferredLightingPS.hlsl samples.
//
// history_/accumulated_ (rgb = radiance, a = variance) is the FEEDBACK TAP,
// not the final image - ATrousPass2 writes it and next frame's reprojection
// reads it, while the image DeferredLightingPS.hlsl samples comes from
// radiance_srv_index_ (the denoised_ output below, written by ATrousPass3).
//
