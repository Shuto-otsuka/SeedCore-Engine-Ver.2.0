...
// Per-pixel ReSTIR reservoir for the single GGX-importance-sampled reflection
// ray ReflectionRayGeneration traces per frame (replacing the old 4-rays-
// averaged-per-frame scheme - see ReflectionRT.hlsl). sample_direction_ is
// the traced reflect direction, kept so a spatial neighbor's sample can be
// re-picked without re-tracing; sample_hit_distance_ is carried through so
// ReflectionDenoiseCS.hlsl's existing hit-point virtual-motion reprojection
// keeps working unchanged downstream. sample_radiance_ is that candidate's
// traced radiance (same convention the old 4-ray average used - no explicit
// BRDF/pdf division, see ReflectionRT.hlsl). sample_m_ is the confidence
// (candidate count folded in so far, clamped by callers) and sample_w_ is the
// final unbiased resampling weight for sample_radiance_.
// 48 bytes - must match the C++ side
// (ReflectionRenderer::reservoirElementSizeInBytes_) byte-for-byte.
...
// Wraps one fresh GGX-importance-sampled candidate as a trivial single-sample
// reservoir: M = 1, and W = 1 because a reservoir of exactly one sample
// resamples itself with probability 1 by definition (wsum == target, so
// W = wsum / (M * target) = 1).
...
// ReflectionRT.hlsl's call site comment.
