...
// GlobalIlluminationRayGeneration traces per frame. sample_position_/
// sample_normal_ are the secondary hit point in world space (kept for a
// future visibility-reuse pass; the combine below does not need them).
// sample_radiance_ is that candidate's already-pdf-divided Monte Carlo
// estimate (what payload.radiance_ used to be written directly). sample_m_ is
// the confidence (candidate count folded in so far, clamped by callers) and
// sample_w_ is the final unbiased resampling weight for sample_radiance_.
// 48 bytes - must match the C++ side
// (GlobalIlluminationRenderer::reservoirElementSizeInBytes) byte-for-byte.
