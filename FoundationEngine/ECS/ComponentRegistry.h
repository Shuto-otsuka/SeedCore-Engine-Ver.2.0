...
namespace SeedCore
{
	...
	*/
	...
	private:
		/**
		* [EN]
		* Debug and Release use different implementations.
		* Debug: kept as a static inline member as before (behavior unchanged).
		* Release: the static initializer generated by the REGISTER_COMPONENT
		* macro can call Register<T>() before registry_ itself is constructed,
		* because initialization order across translation units is unspecified
		* (Static Initialization Order Fiasco). In Release this surfaced due to
		* link order/optimization differences: buckets_ was still unconstructed
		* (size == 0) when operator[] was called, causing an out-of-range access
		* crash (exception). Using a function-local static (Meyer's singleton)
		* guarantees construction on first access.
		*
		* ---------------------------------------------------------------------
		*
		* [JP]
		* Debug/Releaseで実装を分ける。
		* Debug: 従来通り static inline メンバとして直接保持する(挙動を変えない)。
		* Release: REGISTER_COMPONENT マクロが生成する静的初期化子が、TU間の
		* 初期化順序が未規定であることに起因して registry_ 自体の構築より先に
		* Register<T>() を呼んでしまうことがある(Static Initialization Order
		* Fiasco)。Releaseではリンク順序/最適化の違いでこれが顕在化し、buckets_が
		* 未構築(size==0)のまま operator[] が呼ばれ範囲外アクセスによるクラッシュ
		* (例外)が発生していたため、関数ローカル static (Meyerのシングルトン)にして
		* 初回アクセス時に確実に構築されるようにする。
		*/
