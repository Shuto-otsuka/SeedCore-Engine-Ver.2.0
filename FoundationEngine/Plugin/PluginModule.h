	...
	*/
	...
	public:
		...
		*/
		...
		/// [EN] The real DLL path on disk this module is bound to.
		/// [JP] このモジュールが紐づく、ディスク上の実 DLL パス。
		[[nodiscard]] const std::filesystem::path& SourcePath()const;

		/// [EN] Whether the module is currently loaded (handle_ is non-null).
		/// [JP] モジュールが現在ロードされているか（handle_ が非 null か）。
	...
	private:
		...
		/// [EN] Erases every reflection/payload entry this module registered. Must run while the module is still mapped - the registries hold std::function objects whose destructors live inside it.
		/// [JP] このモジュールが登録したリフレクション/ペイロードのエントリを全て削除する。モジュールがまだメモリ上にある間に実行する必要がある - レジストリが保持する std::function のデストラクタはモジュール内にあるため。
		void UnregisterModuleReflection();

		/// [EN] Returns whether id's ComponentMetadata function pointers live inside this module (handle_).
		/// [JP] id の ComponentMetadata の関数ポインタが、このモジュール（handle_）の中にあるかどうかを返す。
		...
		/// [EN] Re-adds every component in capturedComponents_ to its original Actor and restores its captured field values. Clears capturedComponents_ when done.
		/// [JP] capturedComponents_ の各コンポーネントを元の Actor へ再追加し、取得済みのフィールド値を復元する。完了後 capturedComponents_ をクリアする。
